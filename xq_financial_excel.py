...
def fetch_financial_data(symbol_list, folder_path):
    ...
    for symbol in symbol_list:
        ...
        try:
            ...
            balance_data['资产负债率'] = ((balance_data['负债合计'] / balance_data['负债和股东权益总计'])*100).round(2)
            cols1 = ['应付票据及应付账款', '预收款项', '合同负债', '应付职工薪酬', '应交税费', '预计负债','其他流动负债','递延所得税负债']
            balance_data['经营负债'] = balance_data[cols1].sum(axis=1, min_count=1).round(2)
            cols2 = ['短期借款', '交易性金融负债', '应付利息', '一年内到期的非流动负债', '长期借款', '应付债券']
            balance_data['有息负债'] = balance_data[cols2].sum(axis=1, min_count=1).round(2)
            ...
            income_data['毛利率'] = ((1- income_data['其中：营业成本'] / income_data['其中：营业收入'])*100).round(2)
