...
def analysis_data(f):
    ...
    df_selected = df_analysis[["报告期", "ROE","净利率", "毛利率", "营收增长率", "净利润增长率", "资产负债率"]].iloc[:20]
...
def list_industry(dir):
    industries = [name for name in os.listdir(dir) if os.path.isdir(os.path.join(dir, name))]
