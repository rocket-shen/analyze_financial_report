...
symbol = input('输入证券代码（SH/SZ）:')
folder_path = 'D:/雪球数据/个股财务报表'
if not os.path.exists(folder_path):
    os.makedirs(folder_path)

stock_dict = np.load(r'stock_dict.npy', allow_pickle=True).tolist()
stock_name = stock_dict[symbol]
print(stock_name)

file_path = os.path.join(folder_path, f'{symbol}_{stock_name}_财务数据.xlsx')
...
params1 = {'symbol': symbol, 'type': 'all', 'is_detail': 'true', 'count': 30, 'timestamp': int(round(time.time()*1000))}
...
with open("financial.json", "r", encoding="utf-8") as f:
